...
namespace PwdMaker.Services
{
    ...
    {
        /// <summary>作成するパスワードの個数の規定値</summary>
        public static readonly int DEFAULT_NUMBER_VALUE = 6;
        ...
        /// パスワード作成処理を行います。
        /// </summary>
        /// <param name="pwdNumber">パスワードの個数を設定します。</param>
        ...
        /// <remarks></remarks>
        public List<string> MakePassword(int pwdNumber, int pwdLength, bool useNumric, bool useUpperCase, bool useLowerCase, bool useSymbol)
        {
            // パスワード作成処理を行います。
            return MakePasswordCore(pwdNumber, pwdLength, useNumric, useUpperCase, useLowerCase, useSymbol);
        }
        ...
        public List<string> MakePasswordByBatchProcess(int numberValue,int lengthValue)
        {
            // 設定ファイルシングルトンのインスタンスを取得します。
            var css = ConfigSingletonService.GetInstance();

            // パスワード作成処理を行います。
            return MakePasswordCore(numberValue,
                                    lengthValue,
        ...
        /// <returns>処理の成否を返します。</returns>
        /// <remarks>作成したパスワードの文字列リストを返します。</remarks>
        private List<string> MakePasswordCore(int pwdNumber, int pwdLength, bool useNumric, bool useUpperCase, bool useLowerCase, bool useSymbol)
        {
            try
            {
                // パスワードテンプレートの変数の宣言
                ...
                // nudNumberの値の個数を作成します。
                for (int i = 0; i <= pwdNumber - 1; i++)
                {
                    // nudLengthの値の長さを作成します。
                    for (int j = 0; j <= pwdLength - 1; j++)
                    {
                        if (j == 0 && !string.IsNullOrEmpty(firstTemplate))
                        {
                            // パスワード作成処理をします。
        ...
        /// </summary>
        /// <param name="stringList">設定する文字列リスト。</param>
        /// <param name="separator">設定する文字列リストの区切り文字列。</param>
        public void SetClipboard(List<string> stringList, string separator)
        {
            // 文字列を区切り文字にて連結しクリップボードに設定します。
            Clipboard.SetDataObject(string.Join(separator, stringList), true);
        }

        /// <summary>
        /// 文字列リストをテキストファイルに出力します。
        /// </summary>
        /// <param name="stringList">設定する文字列リスト。</param>
        /// <param name="separator">設定する文字列リストの区切り文字列。</param>
        /// <param name="filePath">出力するファイルパス。</param>
        public void Output(List<string> stringList, string separator,string filePath)
        {
            // ファイル出力用のストリームをインスタンス化します。
            using (var sw = new StreamWriter(filePath, false, Encoding.UTF8))
            {
                // ファイルを出力します。
                sw.WriteLine(string.Join(separator, stringList));
